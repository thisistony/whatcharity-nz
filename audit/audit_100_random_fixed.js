...
async function runAudit() {
    ...
    console.log(`Testing 100 random charities...`);
    ...
    try {
        const shuffled = TEST_CHARITIES.sort(() => Math.random() - 0.5).slice(0, 100);
        for (let i = 0; i < shuffled.length; i++) {
            const result = await auditCharity(shuffled[i], browser, i + 1, shuffled.length);
            results.push(result);
